    ...
    if Player and isLoaded then
        if Player.Functions.RemoveMoney('cash', data.price) then
            local metadata = {
                ...
                name = data.name or "Spray Can"
            }
            
            exports.ox_inventory:AddItem(source, 'spraycan', 1, metadata)                        
...
QBCore.Functions.CreateUseableItem('spraycan', function(source, item)
    ...
    if Player and isLoaded then
        if item.metadata then
            TriggerClientEvent('qb-graffiti:client:placeGraffiti', source, item.metadata.model, item.slot)
        else
