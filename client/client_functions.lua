...
function PlaceGraffiti(model, cb)
    ...
    if not HasModelLoaded(modelHash) then
        print("ERROR - Model konnte nicht geladen werden:", modelHash)
        cb(false)
        return
    end
    ...
    if placingObject then
        ...
        CreateThread(function()
            while isPlacing do
                ...
                local graffiti = GetClosestGraffiti(Config.distanceBetweenGraffitis)
        ...
        print("ERROR - CreateObject() fehlgeschlagen für Modell:", modelHash)
        cb(false)
    end
end
